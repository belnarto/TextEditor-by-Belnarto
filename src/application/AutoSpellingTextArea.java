...
public class AutoSpellingTextArea extends StyledTextArea<Boolean> {
	...
	private dictionary.AutoComplete autoComplite;
	private dictionary.Dictionary dict;
	private spelling.SpellingSuggest spellingSuggestion;

	// set up reflection for spelling suggest
	// NB! не понимаю как эта штука работает	
	...
	// NB! не понимаю как эта штука работает
	public AutoSpellingTextArea(dictionary.AutoComplete ac, spelling.SpellingSuggest ss, dictionary.Dictionary dic) {
		...
		this.autoComplite = ac;
		this.spellingSuggestion = ss;
	...
	public StyleSpans<Boolean> checkSpelling() {
		...
		while (matcher.find()) {
			...
			boolean styleClass = dict.isWord(word);
	...
	private void showSuggestions(String word, MouseEvent click) {
		List<String> suggestions = spellingSuggestion.suggestions(word, NUM_SUGGESTIONS);
		...
		item.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent actionEvent) {
				...
				dict.addWord(word);
				((dictionary.Dictionary) autoComplite).addWord(word);
	...
	private void showCompletions(String prefix) {
		...
		if (!prefix.equals("")) {
			...
			options = (autoComplite).predictCompletions(prefix, NUM_COMPLETIONS);
